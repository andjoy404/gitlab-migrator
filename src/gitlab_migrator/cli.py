...
def build_parser():
    ...
    merge_requests = commands.add_parser("migrate-merge-requests")
    merge_requests.add_argument("--days", type=float, default=30)
    merge_requests.add_argument("--reset", action="store_true")
    commands.add_parser("migrate-variables")
    commands.add_parser("migrate-group-variables")
    commands.add_parser("migrate-hooks")
    commands.add_parser("migrate-protection")
    commands.add_parser("export-runners")

    deploy = commands.add_parser("deploy-runners")
    deploy.add_argument("--plan")
    deploy.add_argument("--keys-dir")
    deploy.add_argument("--port", type=int, default=22)
    commands.add_parser("resume-runners")

    export = commands.add_parser("export-pipelines")
    export.add_argument("--days", type=float, default=30)
    export.add_argument("--reset", action="store_true")
    replay = commands.add_parser("replay-pipelines")
    replay.add_argument("--file")
    replay.add_argument("--all-records", action="store_true")

    registry = commands.add_parser("migrate-registry")
    registry.add_argument("--days", type=float, default=30)
    registry.add_argument("--reset", action="store_true")
    retention = commands.add_parser("set-registry-retention")
    ...
    purge = commands.add_parser("purge-registry-images")
    ...
    cancel = commands.add_parser("cancel-pipelines")
